...
# 2. Добавьте бинарную целевую переменную для классификации
df['success'] = (df['outcome'] == 1).astype(int)

# 3. Определите признаки для модели классификации (успех задачи на хосте)
exclude_clf = [
...
# Преобразовать значения к int (если есть NaN — обработать)
for col in mat_cols:
    df[col] = df[col].fillna(-1).astype(int)

# Категориальные признаки — автоматически все object/строковые
cat_features_clf = [c for c in features_clf if df[c].dtype == 'object' or c.startswith('mat_')]

# 4. Разделите выборку (train/test)
...
print("\n=== Модель для ПРОГНОЗА УСПЕША на хосте ===")
...
# 7. Определите признаки для регрессии (прогноз времени — только признаки задачи и матрицы)
